...
function drawAudioPractice(preview=null){if(deferFeedback('audio',drawAudioPractice))return;
 ...
 if(preview){el.querySelectorAll('button,input,select').forEach(e=>e.disabled=true);return;}if(!locked)playAudioPractice();
}
function advanceAudioPractice(){if(deferFeedback('audio',advanceAudioPractice))return;
