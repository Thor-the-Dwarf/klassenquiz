...
async function drawDiscussion(){if(deferFeedback('discussion',drawDiscussion))return;
 ...
 if(!valid())return;const next=await decodedSlide(blob,topic.title+' – Folie '+(index+1));if(!valid()){URL.revokeObjectURL(next.url);return}const oldUrl=discussion.url;discussion.url=next.url;$('#discussion-stage').replaceChildren(next.img);discussion.displayed={kind:'discussion',session:state.id,topic:topic.id,index};$('#discussion-stage').closest('.discussion-view').querySelector('.slide-navigation').innerHTML=nextNavigation;$('#discussion-stage').closest('.discussion-view').setAttribute('aria-label','Gemeinsame Präsentation: '+topic.title);$('#discussion-stage').setAttribute('aria-busy','false');if(oldUrl)URL.revokeObjectURL(oldUrl);void preloadSlides(data,index,topic.slides,valid);
 }catch(e){if(valid()){$('#discussion-stage').setAttribute('aria-busy','false');if(!$('#discussion-stage img'))$('#discussion-stage').textContent=e.message;discussion.renderKey=null}}
}
