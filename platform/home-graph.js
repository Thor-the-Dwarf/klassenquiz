...
async function drawHomeGraph(){
 ...
 content.innerHTML=`<section class="home-graph core-live"><div class="home-graph-heading"><h1>Wissenscluster</h1><span id="core-count" role="status">Wird geladen …</span></div><div class="core-cluster-tools">${coreModes.map(([key,label,color])=>`<button class="secondary core-cluster-toggle" data-core-mode="${key}" style="--cluster-color:${color}" aria-pressed="${coreDemo.active.has(key)}">${label}</button>`).join('')}</div><div class="core-search-row"><input id="core-search" type="search" placeholder="Cluster oder Core suchen" aria-label="Cluster und Cores durchsuchen" value="${esc(coreDemo.search)}"><button class="secondary" id="core-overview">Übersicht</button></div><p id="core-status" class="core-demo-note"></p><div class="cluster-group-legend" aria-label="Gruppen" hidden></div><div class="core-stage"><canvas class="core-webgl" tabindex="0" aria-label="Wissenscluster. Pfeiltasten wählen, Eingabe bestätigt. Plus und Minus zoomen. Cores erscheinen im ausgewählten Cluster beim Hineinzoomen."></canvas><div class="core-hover" hidden></div><div class="cluster-zoom"><button class="secondary" id="cluster-zoom-out" aria-label="Herauszoomen">−</button><output id="cluster-zoom-level">100 %</output><button class="secondary" id="cluster-zoom-in" aria-label="Hineinzoomen">+</button></div></div><p id="cluster-focus-status" class="cluster-focus-status" aria-live="polite">Cluster auswählen und hineinzoomen</p><div class="core-legend"><span>● Unbearbeitet</span><span class="core-zero">● 0 %</span><span class="core-half">● 50 %</span><span class="core-full">● 100 %</span><span>Linien = gemeinsame Cores</span></div><section id="core-detail" aria-live="polite"><p>Wähle einen Cluster. Beim Hineinzoomen werden seine Cores sichtbar.</p></section></section>`;
...
function createCoreGraph(root){
 ...
 function render(){
  ...
  const legend=root.querySelector('.cluster-group-legend'),markup=groupLabels.join('');if(legend.innerHTML!==markup)legend.innerHTML=markup;legend.hidden=!groupCount;
