...
function saveNavigation(){
 ...
 storage.set(key,{coreGraph:{active:[...coreDemo.active],selected:coreDemo.selected,search:coreDemo.search,offset:coreDemo.offset,center:coreDemo.center},view,selected,expanded:[...expanded],chartMode,chartDifficulty,chartSelection:[...chartSelection],
...
async function restoreNavigation(){
 ...
 try{
  if(!saved){coreDemo.active.clear();coreDemo.selected=null;coreDemo.search='';coreDemo.offset=0;coreDemo.center=null;await drawView();return;}
  coreDemo.active=new Set((saved.coreGraph?.active||[]).filter(k=>['arp','lf','exam'].includes(k)).slice(-1));coreDemo.selected=typeof saved.coreGraph?.selected==='string'?saved.coreGraph.selected:null;coreDemo.search=saved.coreGraph?.search||'';coreDemo.offset=saved.coreGraph?.offset||0;coreDemo.center=saved.coreGraph?.center||null;
...
async function navigationHome(){
 ...
 try{
  ...
  coreDemo.active.clear();coreDemo.selected=null;coreDemo.search='';coreDemo.offset=0;coreDemo.center=null;hostNavigation.panel=null;hostNavigation.base=null;hostNavigation.positions.clear();expanded.clear();chartSelection.clear();grantDraft=null;
