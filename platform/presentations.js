...
async function openPresentation(id){const t=(boot.presentations||[]).find(t=>t.id===id);if(!t?.ready)return;await capture();slidesView.topic=t;slidesView.index=0;view='presentation';drawTabs();drawSidebar();await drawPresentation()}
async function drawPresentation(){
 ...
 $('#content').innerHTML=`<section class="presentation-view"><div class="row"><h1>${esc(t.title)}</h1><button data-presentation-pdf>PDF herunterladen</button></div><div class="slide-stage" id="slide-stage" role="status">Folie wird geladen …</div><div class="row slide-navigation"><button data-slide-prev ${slidesView.index?'':'disabled'}>Zurück</button><span>Folie ${slidesView.index+1} / ${t.slides}</span><button data-slide-next ${slidesView.index<t.slides-1?'':'disabled'}>Weiter</button></div></section>`;
